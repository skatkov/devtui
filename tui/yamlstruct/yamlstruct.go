...
func (m YamlStructModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "v":
			...
			isValid := isValidYAML(content)
			m.setContent(content)

			if isValid {
				cmds = append(cmds, m.showStatusMessage(ui.PagerStatusMsg{Message: "Converted YAML to Go struct"}))
			} else {
				cmds = append(cmds, m.showStatusMessage(ui.PagerStatusMsg{Message: "Invalid YAML"}))
			}
		case "c":
			c := clipboard.New()
	...
	case editor.EditorFinishedMsg:
		...
		isValid := isValidYAML(msg.Content)
...
func (m *YamlStructModel) setContent(content string) {
	m.content = content
	m.converted_content = convertYAMLToStruct(content)
...
func (m YamlStructModel) statusBarView(b *strings.Builder) {
	...
	if showStatusMessage {
		...
		note = "Press 'v' to paste YAML to convert to Go struct"
	}
...
func convertYAMLToStruct(content string) string {
	if !isValidYAML(content) {
		return "Invalid YAML"
	}
	...
	// Observe the YAML content directly, no need to convert to JSON first
	err := generator.ObserveYAMLReader(strings.NewReader(content))
	if err != nil {
		return "Error processing YAML: " + err.Error()
	}
