...
func (m HTMLFormatterModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "v":
			...
			cmds = append(cmds, m.showStatusMessage(ui.PagerStatusMsg{Message: "Formatted HTML"}))
		...
		case "c":
			...
			cmds = append(cmds, m.showStatusMessage(ui.PagerStatusMsg{Message: "Copied formatted HTML"}))
		case "?":
			m.toggleHelp()
		}
...
func (m *HTMLFormatterModel) showStatusMessage(msg ui.PagerStatusMsg) tea.Cmd {
...
func (m *HTMLFormatterModel) setContent(content string) {
	m.content = content
	m.formattedContent = gohtml.Format(content)

	var buf bytes.Buffer
	_ = quick.Highlight(&buf, m.formattedContent, "html", "terminal", "nord")
	m.viewport.SetContent(buf.String())
}

func (m *HTMLFormatterModel) setSize(w, h int) {
	m.viewport.Width = w
...
func (m *HTMLFormatterModel) toggleHelp() {
...
func (m HTMLFormatterModel) statusBarView(b *strings.Builder) {
	const (
	...
	if showStatusMessage {
		...
		note = "Press 'v' to paste HTML to format"
	}
