...
in a formatted table. Input can be a string argument or piped from stdin.`,
   ...
   cat article.txt | devtui count`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if countJSON {
			fmt.Fprintf(cmd.OutOrStdout(), `{"characters":%d,"spaces":%d,"words":%d}`, stats.Characters, stats.Spaces, stats.Words)
		} else {
...
func init() {
	countCmd.Flags().BoolVarP(&countJSON, "json", "j", false, "JSON output for structured data")
